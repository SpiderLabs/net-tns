  ...
  module TTI
    describe AuthenticationX86 do
      ...
      context "when serializing authentication messages" do
        subject {AuthenticationX86.new( :username => "system" )}

        before :each do
          subject.sequence_number = 3 # Sequence number in example
        end

        it "should properly serialize an x86 authentication request" do
          ...
          expect(subject.to_binary_s).to eql_binary_string( auth_request_x86 )
        end

        it "should properly serialize an x86 authentication request when calling #add_parameter" do
